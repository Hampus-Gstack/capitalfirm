// Automatic Blog Content Generator for Capital Raising
...
function generateReadTime(contentLength: number): string {
  const wordsPerMinute = 200;
  const minutes = Math.ceil(contentLength / wordsPerMinute);
  return `${minutes} min read`;
}

function generateContent(topic: any, title: string): string {
...
export function generateBlogPost(id: number): BlogPost {
